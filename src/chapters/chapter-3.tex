...
\par Meskipun program solusi juri dan solusi peserta dieksekusi pada komputer yang sama. Waktu eksekusi kedua program tersebut dapat sedikit berbeda karena adanya perbedaan implementasi antara solusi peserta dan juri. Solusi peserta bisa saja lebih lambat atau lebih cepat dibandingkan dengan solusi juri. Oleh karena itu, perlu adanya faktor toleransi terhadap waktu eksekusi peserta. Solusi peserta dapat diterima jika waktu eksekusinya masih berada pada batas toleransi dari solusi juri. Secara matematis, solusi peserta akan diterima jika memenuhi persamaan berikut $ T_{peserta} < (1 + \gamma) \times T_{juri}$, dengan $\gamma$ adalah nilai dari faktor toleransi.
...
\par \textit{Cgroup}, \textit{chroot} dan \textit{namespace} merupakan suatu fitur pada Linux yang memungkinkan penggunanya untuk mengeksekusi program secara terisolasi. \textit{Cgroup} dapat memberikan batasan \textit{resource} seperti IO, memori dan CPU kepada suatu proses sehingga tidak membebani proses lain. \textit{Chroot} memberikan isolasi \textit{filesystem} pada suatu proses sehingga proses yang berjalan di dalam \textit{chroot} tidak dapat mengakses \textit{filesystem} yang berada di luar \textit{chroot}. \textit{Namespace} memisahkan proses pada Linux menjadi beberapa partisi. Proses pada sebuah partisi hanya dapat mengetahui proses lain yang berada dalam partisi tersebut. Dengan menggabungkan ketiga fitur Linux ini, dapat dibuat \textit{sandbox} yang cukup aman untuk mengeksekusi jawaban peserta.

\par Teknik ini sudah digunakan oleh teknologi \textit{container} untuk mengisolasi program. Dengan menggunakan teknik ini, penilaian jawaban peserta dapat dilakukan secara aman dengan sedikit \textit{overhead}. Oleh karena itu, isolasi program dengan \textit{cgroup}, \textit{chroot} dan \textit{namespace} dipilih dalam Tugas Akhir ini.
